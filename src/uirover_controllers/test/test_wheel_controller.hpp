...
#ifndef TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_WHEEL_CONTROLLER_HPP_
#define TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_WHEEL_CONTROLLER_HPP_
...
#include "uirover_controllers/wheel_controller.hpp"
...
using ControllerStateMsg = uirover_controllers::WheelController::ControllerStateMsg;
using ControllerReferenceMsg = uirover_controllers::WheelController::ControllerReferenceMsg;
using ControllerModeSrvType = uirover_controllers::WheelController::ControllerModeSrvType;
...
class TestableWheelController : public uirover_controllers::WheelController
{
  FRIEND_TEST(WheelControllerTest, all_parameters_set_configure_success);
  FRIEND_TEST(WheelControllerTest, activate_success);
  FRIEND_TEST(WheelControllerTest, reactivate_success);
  FRIEND_TEST(WheelControllerTest, test_setting_slow_mode_service);
  FRIEND_TEST(WheelControllerTest, test_update_logic_fast);
  FRIEND_TEST(WheelControllerTest, test_update_logic_slow);

public:
  controller_interface::CallbackReturn on_configure(
    const rclcpp_lifecycle::State & previous_state) override
  {
    auto ret = uirover_controllers::WheelController::on_configure(previous_state);
...
class WheelControllerFixture : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerReferenceMsg>(
      "/test_wheel_controller/commands", rclcpp::SystemDefaultsQoS());
    ...
    slow_control_service_client_ = service_caller_node_->create_client<ControllerModeSrvType>(
      "/test_wheel_controller/set_slow_control_mode");
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_wheel_controller")
  {
    ASSERT_EQ(
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_wheel_controller/state", 10, subs_callback);
...
protected:
  ...
  std::unique_ptr<TestableWheelController> controller_;
...
#endif  // TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_WHEEL_CONTROLLER_HPP_
