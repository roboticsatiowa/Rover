...
if [ ! -L /dev/ttyTEENSY ]; then
    echo -e "${RED}ERROR: /dev/ttyTEENSY does not exist. Please plug in the Teensy.${NC}"
    exit 1
fi

(cd "$SCRIPT_DIR/../../teensy" && platformio run --target upload) || exit 1